...
# Cycle Counter
class CycleCounter(Module):
    def __init__(self, master, slave=None, max=255):
        if slave is None:
            slave = wb.Interface(data_width=master.data_width, adr_width=master.adr_width)

        self.bus = slave

        self.comb += master.connect(slave)
        self.counter = Signal(max=max)
        _last = Signal(reset=0)
        self.sync += [
            _last.eq(master.stb),
            If(master.stb & ~_last, self.counter.eq(0)),
            If(master.stb & _last, self.counter.eq(self.counter + 1))
        ]
